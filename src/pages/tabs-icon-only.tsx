          ...
          <ul className="flex relative border-b-2 border-gray-200 list-none p-0 m-0">
            {tabs.map((tab) => (
              <li key={tab.id} ref={(el) => (tabRefs.current[tab.id] = el)} className="flex-1">
                <a
                  href="#"
                  className={`flex items-center justify-center relative px-4 py-3 text-base font-normal transition-colors duration-300 text-center whitespace-nowrap w-full group ${
                    activeTab === tab.id
                ...
                >
                  {tab.icon}
                  <div className="absolute top-full mt-2 px-2 py-1 bg-gray-800 text-white text-sm rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-10">
                    {tab.label}
                  </div>
                </a>
              </li>
