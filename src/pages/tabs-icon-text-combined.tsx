          ...
          <ul className="flex relative border-b-2 border-gray-200 list-none p-0 m-0">
            {tabs.map((tab) => (
              <li key={tab.id} ref={(el) => (tabRefs.current[tab.id] = el)} className="flex-1">
                <a
                  href="#"
                  className={`flex flex-col items-center justify-center relative px-3 py-2 text-sm font-normal transition-colors duration-300 text-center whitespace-nowrap w-full ${
                    activeTab === tab.id
...
export default TabsIconTextCombined;