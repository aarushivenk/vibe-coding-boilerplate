...
const TabsIconWithHoverTitle = () => {
  ...
  useEffect(() => {
    if (hoveredTab) {
      updateSlider(hoveredTab);
      const timer = setTimeout(() => updateSlider(hoveredTab), 450);
              ...
              <li key={tab.id} ref={(el) => (tabRefs.current[tab.id] = el)} className="flex-none">
                ...
                >
                  ...
                  <span className={`inline-block overflow-hidden whitespace-nowrap transition-all duration-[400ms] ${
                    hoveredTab === tab.id ? 'max-w-[80px] ml-2' : 'max-w-0'
                  }`}>
                    {tab.label}
                  </span>
                </a>
...
export default TabsIconWithHoverTitle;